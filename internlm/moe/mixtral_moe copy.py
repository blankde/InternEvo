...
class MixtralMoE(BaseMoELayer):
    ...
    def __init__(
        ...
        super().__init__(
            nn.Linear(hidden_size, num_experts, bias=False, dtype=dtype, device=device),
            MegaBlockFeedForward(
                hidden_size,
                self.ffn_dim // tp_size * num_experts,
                parallel_mode,
                device,
                dtype,
            ),
    ...
    def forward(self, *inputs) -> torch.Tensor:
        ...
        with torch.no_grad():
            (indices, bin_ids, bins, padded_bins, _) = self.indices_and_padded_bins(selected_experts)
