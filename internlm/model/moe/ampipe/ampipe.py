...
from internlm.core.context import random, ParallelMode

from .fa_helper import flash_attn_bwd, flash_attn_fwd
import os
...
def bias_dropout_add_ln_bwd(ctx, grad_ln_outs, grad_ln_ins, ln):
    grad_fusion, grad_ln_weight = ln.explicit_bwd(ctx, grad_ln_outs)
    with torch.enable_grad():
        ctx.ln_input.backward(grad_fusion + grad_ln_ins)

    return grad_ln_weight, ctx.inp.grad, ctx.residual.grad, ctx.bias.grad
    ...
    def forward(ctx, q, k, v, hidden_states, ln_weight, ln_bias, proj_bias, non_params):
        ...
        for c in range(pipe_degree):
            with torch.cuda.stream(get_comp0()):
                ...
                # moe.dummy_moe_loss(loss_ctx, scoreses[c], tokens_per_experts[c])
